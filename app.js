...
app.post("/search", async (req, res) => {
  ...
  try {
    ...
    const artistKey = await fetch(
      `https://api.setlist.fm/rest/1.0/search/artists?artistName=${req.body.input}&p=1&sort=relevance`,
      {
        method: "GET",
        headers: {
...
app.post("/gigs", async (req, res) => {
  ...
  const setlists = await fetch(
    `https://api.setlist.fm/rest/1.0/artist/${req.body.artistMbid}/setlists?p=1`,
    {
      method: "GET",
      headers: {
        "x-api-key": process.env.setlistfmKey,
...
app.post("/spotify", (req, res) => {
  setlistNum = req.body.setlistNum;
  ...
  if (state === null) {
    ...
    for (let i = 0; i < gig.setlist[setlistNum].sets.set[0].song.length; i++) {
      if (gig.setlist[setlistNum].sets.set[0].song[i].cover) {
        const getTrackIds = await fetch(
          `https://api.spotify.com/v1/search?q=artist%3A${gig.setlist[setlistNum].sets.set[0].song[i].cover.name}+track%3A${gig.setlist[setlistNum].sets.set[0].song[i].name}&type=track&limit=5`,
          {
            method: "GET",
            headers: {
              Authorization: "Bearer " + data.access_token,
        ...
        const getTrackIds = await fetch(
          `https://api.spotify.com/v1/search?q=artist%3A${gig.setlist[
            setlistNum
          ].artist.name.replace("#", "")}+track%3A${gig.setlist[
            setlistNum
          ].sets.set[0].song[i].name.replace("#", "")}&type=track&limit=5`,
          {
            method: "GET",
            headers: {
              Authorization: "Bearer " + data.access_token,
