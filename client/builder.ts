...
const RESULTS_CONTAINER =
  document.querySelector<HTMLElement>('.js-results-list');
const SELECTED_SETLIST_CONTAINER = document.querySelector(
  '.js-selected-setlist-container'
);
...
const SELECTED_SETLIST = document.querySelector<HTMLElement>(
  '.js-selected-setlist'
);
...
const getData = async (url: string, value: string) => {
  try {
...
const populateArtists = (artistData: SetlistFmData) => {
  ...
  if (RESULTS_CONTAINER && artistData.artist) {
    ...
    for (const artist of artistData.artist) {
      createButton(
        RESULTS_CONTAINER,
        artist.name,
        'js-artist-result text-left lg:hover:text-gray-400 transition-colors',
        {
...
const populateSetlists = (setlistsData: Setlist[]) => {
  if (!RESULTS_CONTAINER) return;
  RESULTS_CONTAINER.innerHTML = '';
  for (const setlist of setlistsData) {
    const buttonText = `${setlist.venue.name}, ${setlist.venue.city.name}, ${
      setlist.venue.city.state
    } ${setlist.eventDate.replace(/-/g, ' ')}`;
    createButton(
      RESULTS_CONTAINER,
      buttonText,
      'js-setlist-result text-left text-2xl cursor-pointer lg:hover:text-gray-400 transition-colors',
      {
        'data-id': setlist.id,
      }
    );
...
const addSetlistListeners = (setlistsData: Setlist[]) => {
  const BUTTONS =
    RESULTS_CONTAINER?.querySelectorAll<HTMLButtonElement>(
      '.js-setlist-result'
    );
  if (!BUTTONS) return;
...
const getMatchedData = (button: HTMLButtonElement, setlistsData: Setlist[]) => {
  return setlistsData.find((setlist) => setlist.id === button.dataset.id);
};

const loadSelectedSetlist = (data: Setlist | undefined) => {
  if (!data) return;

  if (SELECTED_ARTIST) SELECTED_ARTIST.innerHTML = data.artist.name;
  if (SELECTED_PLACE)
  ...
  if (CREATE_FORM_INPUT) CREATE_FORM_INPUT.value = data.id;
